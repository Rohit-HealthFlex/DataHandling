...
data_path = "data/test/Custom Sensor Script/knee_flex_cmp.csv"

req_cols = "Time,Device name,Acceleration X(g),Acceleration Y(g),Acceleration Z(g),Angle X,Angle Y,Angle Z,Magnetic field X,Magnetic field Y,Magnetic field Z"
...
for row in streamer.stream():
    ...
    angle_info = preset_obj.get_angle(angle_info, pos_info)
...
data_path = "data/test/Custom Sensor Script/test_store_sumo_squat_360.csv"
...
for row in streamer.stream():
    ...
    direction = preset_obj.get_direction(rot_info)
    position = preset_obj.get_position(pos_info)
    start_pos_info = preset_obj.get_starting_pos(angle_info)
    ...
    # applying rules
    rule_flag, error_info = rule0.apply_rule(
        direction, position, angle_info, dist_info, start_pos_info)
    print(rule_flag, error_info, "\n\n")
