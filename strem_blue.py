...
class GyroProcessor():
    def __init__(self, data=None):
        ...
        transformed_hex_values = [val if val <=
                                  127 else (256-val)*-1 for val in hex_data]
        for i in range(0, 3):
            if i == 0:
                # "Acceleration"
                self.ax = float((int(transformed_hex_values[i+1]) << 8) | (
                    int(transformed_hex_values[i]) & 255))/32768*(16*9.8)
                self.ay = float((int(transformed_hex_values[i+3]) << 8) | (
                    int(transformed_hex_values[i+2]) & 255))/32768*(16*9.8)
                self.az = float((int(transformed_hex_values[i+5]) << 8) | (
                    int(transformed_hex_values[i+4]) & 255))/32768*(16*9.8)
...
MAC_ble_device = "FF:21:24:A3:8A:02"
addr = False
try:
    # it connects to the ble device
    ble_device = Peripheral(MAC_ble_device, ADDR_TYPE_RANDOM,
                            0).withDelegate(DeviceDelegate())
    ...
    while not addr:
        try:
            ble_device = Peripheral(
                MAC_ble_device, ADDR_TYPE_RANDOM, 0).withDelegate(DeviceDelegate())
            addr = ble_device.addr
        except:
            pass
        time.sleep(0.1)
